...
const assetFiles = files.filter(
  (f) =>
    !f.startsWith("ocr/") &&
    !["sw.js", "offline-ocr.json", "version.json", ".nojekyll"].includes(f),
);
...
self.addEventListener('fetch',event=>{const url=new URL(event.request.url);if(url.pathname===new URL('version.json',ROOT).pathname)return;if(event.request.method!=='GET'||url.origin!==ROOT.origin||!url.pathname.startsWith(ROOT.pathname))return;
